 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`rtmcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`rtmcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'rtmcore-' + name : 'rtmcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/rtmcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external rtmcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:rtmcore-lib -o rtmcore-lib.js';
    }
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push Raptor3um $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
